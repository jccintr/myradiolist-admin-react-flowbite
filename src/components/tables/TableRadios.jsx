    ...
    <Table hoverable >
        <Table.Head>
          <Table.HeadCell>Nome</Table.HeadCell>
           ...
           <Table.HeadCell>Categorias</Table.HeadCell>
        ...
        {radios.map((radio) => (
             <Table.Row className="bg-white" key={radio.id}>
                  <Table.Cell className='text-slate-950'>{radio.name}</Table.Cell>
                   <Table.Cell className='text-slate-950'>{radio.city.name+' - '+radio.city.state}</Table.Cell>
                   <Table.Cell className='text-slate-950'>{radio.categories.map((category) => (
                    <button
                    key={category.id}
                    onClick={() => {}}
